...
void update_HEMS(HEMS* engine) {
  ...
  for (temp_counter = 0; temp_counter < NUM_THERMISTORS; temp_counter++) {
    float thermistor_ratio = ADC_read(engine->bus, engine->ADC_device_address[0], temp_counter + 1);

    //Calculate thermistor resistance
    float thermistance = thermistor_ratio * REFERENCE_RESISTANCE / (1 - thermistor_ratio);
