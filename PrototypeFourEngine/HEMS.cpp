...
HEMS* initialize_HEMS(uint8_t I2C_BUS, uint8_t I2C_DIP) {
  HEMS* engine = (HEMS*)malloc(sizeof(HEMS));
  engine->bus = I2C_BUS;
...
void update_HEMS(HEMS* engine) {
  ...
  for (int temp_counter = 0; temp_counter < NUM_THERMISTORS; temp_counter++) {
    ...
    if (thermistor_ratio > 100) {
      ...
      new_temperature = THERMISTOR_BETA / (log(thermistance) - THERMISTOR_OFFSET) - 272;
      ...
      new_temperature = -100; //This is an error signal; it means that the thermistor is disconnected.
